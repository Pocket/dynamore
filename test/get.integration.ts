...
import { seedData } from './fixtures/test-hash-table-seed';
...
describe('DynamoreGet', () => {
  ...
  const table = dynamore(dynamodb)('test-hash-table');
  beforeAll(async () => {
    await truncateTable('test-hash-table', client);
    await seed(client, seedData);
  });
  describe('find', () => {
    it('gets all available attributes for a key', async () => {
      const res = await table.find({ id: 'abc123' }).send();
    ...
    it('returns undefined Item if key does not exist', async () => {
      const res = await table.find({ id: 'notreal' }).send();
      expect(res.Item).toBeUndefined;
    });
    it('selects a specific attribute', async () => {
      const res = await table.find({ id: 'hij789' }).select('species').send();
  ...
  describe('findMany', () => {
    it('returns multiple keys and all their attributes', async () => {
      const res = await table
